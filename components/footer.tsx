...
export default function Footer() {
  ...
  /* On main page: barcode + links use V as flag. Pattern sets --barcode-v-offset (15 V widths → 16th V), --pattern-left, --pattern-width. C of CONTACT aligns to 16th V. Barcode fixed size, no scaling. */
  const useVFlag = isMainPage;
  ...
  return (
    <footer className={`fixed bottom-0 left-0 w-full font-blurlight z-50 pointer-events-auto px-4 sm:px-6 md:px-[4vw] pt-12 sm:pt-16 md:pt-20 pb-4 sm:pb-5 md:pb-6 ${((isArchitecturePage || isArtPage) && isExpandedViewOpen) ? 'bg-transparent' : isMainPage ? 'bg-[#554943]' : isFooterPage ? 'bg-[#554943]' : 'bg-[#554943]'}`}>
      <div
        className={`absolute flex flex-col space-y-2 ${useVFlag ? 'items-end' : 'items-center'}`}
        style={{
          bottom: shouldLowerFooter ? '1rem' : '68px',
          ...(useVFlag
            ? { left: 'var(--pattern-left, calc(50vw - 6rem))', width: 'var(--pattern-width, 50vw)' }
            : { right: '120px' }),
        }}
      >
        <div
          className={`flex flex-col ${useVFlag ? 'items-end w-fit' : 'items-center w-full'}`}
          style={
            useVFlag
              ? { paddingLeft: 'var(--barcode-v-offset, 26rem)' }
              : { maxWidth: '350px' }
          }
        >
          <svg
            ...
            style={useVFlag ? { width: '367px', height: 'auto' } : { width: '100%', height: 'auto', maxWidth: '100%' }}
          >
          ...
          {/* C of CONTACT aligned to same V as barcode left edge */}
          <div className="flex items-center justify-end text-[#fec776] text-[5.5vw] sm:text-[4.5vw] md:text-[26px] font-medium leading-none gap-2 mt-2">
            <Link href="/contact" className="cursor-pointer text-[#fec776] no-underline hover:text-[#fec776] transition-colors duration-200">
              CONTACT
            </Link>
            <span className="text-[#fec776]">/</span>
            <Link href="/about" className="cursor-pointer text-[#fec776] no-underline hover:text-[#fec776] transition-colors duration-200">
              ABOUT
            </Link>
            <span className="text-[#fec776]">/</span>
            <Link href="/news" className="cursor-pointer text-[#fec776] no-underline hover:text-[#fec776] transition-colors duration-200">
              NEWS
            </Link>
          </div>
        </div>
